...
makemodelArray<- function(dataset,repeats)
{
  ...
  for(i in 1:repeats){
   ...
   # claseTest<- train[,c("tip")];
   # train <- subset(train, select=- tip);
    svm_model <- svm(tip~.,data=train,kernel="radial",cost=10,gamma=0.1);
    clase <- test[,c("tip")];
    test <- subset(test, select=- tip);
    pred <- predict(svm_model,test);
    accuracies[i] <- classAgreement(table(pred,clase))$diag;
...
file1<- read.table("d:/programming/r/data/thresh213scan1.txt",header=TRUE)
file2<- read.table("d:/programming/r/data/thresh215scan1.txt",header=TRUE)
file3<- read.table("d:/programming/r/data/thresh217scan1.txt",header=TRUE)
file4<- read.table("d:/programming/r/data/thresh220scan1.txt",header=TRUE)
file5<- read.table("d:/programming/r/data/thresh222scan1.txt",header=TRUE)
file6<- read.table("d:/programming/r/data/thresh223scan1.txt",header=TRUE)
file7<-  read.table("d:/programming/r/data/thresh225scan1.txt",header=TRUE)
file8<-  read.table("d:/programming/r/data/thresh230scan1.txt",header=TRUE)